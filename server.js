...
const DEFAULT_APP_URL = envValue("APP_URL", "PUBLIC_URL") || "https://your-app.vercel.app";
...
const STATIC_EXTENSIONS = /\.(html|css|js|ico|png|jpg|jpeg|gif|svg|woff|woff2|ttf|eot|map|json)$/i;

app.use(async (req, res, next) => {
    // Skip DB check for static file requests
    ...
    try {
        ...
        res.status(500).json({
            success: false,
            message: "Database connection failed. Check DATABASE_URL in Vercel environment variables."
        });
    }
});
...
async function startServer() {
    try {
        ...
        if (!envValue("TOKEN_SECRET")) {
            console.log("TOKEN_SECRET is not set. Set it in Vercel environment variables for stable secure admin sessions.");
        }
